...
data1 = pd.read_csv('../plot_demo_files/thruster_speed_n_trajectory_length/thruster_speed_n_0.1_trajuectory_length.csv')
x1 = data1.iloc[:, 0].values
df1 = pd.DataFrame({r'Iterations$(10^6)$': x1, "Episode steps": exponential_moving_average(data1.iloc[:, 4].values)})
df1['parameter'] = r"$\delta$=0.1"

data1 = pd.read_csv('../plot_demo_files/thruster_speed_n_trajectory_length/thruster_speed_n_0.5_trajuectory_length.csv')
x1 = data1.iloc[:, 0].values
df2 = pd.DataFrame({r'Iterations$(10^6)$': x1, "Episode steps": exponential_moving_average(data1.iloc[:, 4].values)})
df2['parameter'] = r"$ \delta $=0.5"

data1 = pd.read_csv('../plot_demo_files/thruster_speed_n_trajectory_length/thruster_speed_n_1.0_trajuectory_length.csv')
x1 = data1.iloc[:, 0].values
df3 = pd.DataFrame({r'Iterations$(10^6)$': x1, "Episode steps": exponential_moving_average(data1.iloc[:, 4].values)})
...
# 画图
# df = df_
sns.lineplot(data=df, x=r'Iterations$(10^6)$', y="Episode steps", hue="parameter",estimator='mean')
...
# plt.savefig('thruster_speed_with_penalty_trajectory_length.png', dpi=300, bbox_inches='tight')
plt.show()
haha=True
