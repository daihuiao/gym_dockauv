...
data1 = pd.read_csv('../plot_demo_files/2024_demo1/thruster_penalty_tau0.9.csv')
x1 = data1.iloc[:, 0].values
df1 = pd.DataFrame({r'Iterations$(10^6)$': x1, "Episode Return": exponential_moving_average(data1.iloc[:, 4].values)})
df1['parameter'] = r"$\tau$=0.9"

data1 = pd.read_csv('../plot_demo_files/2024_demo1/thruster_penalty_tau0.5.csv')
x1 = data1.iloc[:, 0].values
df2 = pd.DataFrame({r'Iterations$(10^6)$': x1, "Episode Return": exponential_moving_average(data1.iloc[:, 4].values)})
df2['parameter'] = r"$ \tau $=0.5"
...
# 画图
# df = df_
sns.lineplot(data=df, x=r'Iterations$(10^6)$', y="Episode Return", hue="parameter",estimator='mean')
...
# plt.savefig('return_thruster_penalty.png', dpi=300, bbox_inches='tight')
plt.show()
haha=True
