        ...
        for K, MODEL in enumerate(MODELS):
            for GYM in GYM_ENV:
                ...
                for i in range(ncount):
                    ...
                    new_obs, rewards, dones, infos = env.step(np.array([yaw_control, 0, 430]))
                    yaw_to_target = env.yaw_to_target()
                ...
                env.trajectory_in_current_(position_0
                                          , prefix=prefix + f"/haha",
                                          args={
                                                "start_point":[0,-100,0],"goal_point":[0,100,0],
                                           ...
                                           position1=position,
                                           args1={"cmap":True,"cmap_value":thruster,
                                                  "annotated": [f"steps:{PID_steps}\n path length:{round(10 * PID_total_distance_moved, 1)}",
                                                                f"steps:{RL_steps} \n path length:{round(10 * RL_total_distance_moved, 1)}"],
