...
class remus100:
    ...
    def dynamics(cls, eta, nu, u_actual, u_control, sampleTime,nu_c):
        ...
        """
        这里 Dnu_c 代表由洋流引起的加速度变化，它是一个六维向量。这个向量的每个分量对应 AUV 运动学中的
        一个方向或者旋转。
        
        nu[5] * v_c: 这个分量计算了洋流的侧向速度（v_c）和 AUV 的偏航角速度（nu[5]）的乘积。
        在物理上，这表示了由于船体旋转而在船体的纵轴上引起的加速度变化。
        
        -nu[5] * u_c: 这个分量是洋流的纵向速度（u_c）和 AUV 的偏航角速度（nu[5]）的乘积的负值。
        这代表了由于船体旋转而在船体的横轴上引起的加速度变化的反方向。
        
        其余的分量被设为零，因为在这种情况下，我们只考虑了由于洋流在平面运动上造成的影响，
        而忽略了垂直方向或其他旋转方向的影响。
        
        这种计算方法是基于物理原理的。当一个物体（在这个例子中是 AUV）在水流中移动时，
        其运动不仅受到水流速度的直接影响，还受到由于其自身旋转导致的相对水流速度的影响。
