...
if __name__ == "__main__":
    ...
    used_TRAIN_CONFIG["thruster_min"] = used_TRAIN_CONFIG["thruster"] - 1
    #计算二范数
        ...
        for i in range(1, 500):
            for j in range(0, 1):
                try:
                    ...
                    env.reset()
                    env.trajectory_in_current(epi_stor.positions, prefix=used_TRAIN_CONFIG["save_path_folder"] + f"/fig_episode_{i}_process{j}")
                except:
                    pass
    else:
        # ---------- VIDEO GENERATION ----------
